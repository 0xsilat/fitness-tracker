...
func TestOptionalInt(t *testing.T) {
	got, err := optionalInt("12")
	if err != nil || got != 12 {
		t.Fatalf("got=%d err=%v", got, err)
	}
	got, err = optionalInt("")
	if err != nil || got != 0 {
		t.Fatalf("blank got=%d err=%v", got, err)
	}
	if _, err := optionalInt("nope"); err == nil {
		t.Fatal("expected invalid number error")
	}
	if _, err := optionalInt("-1"); err == nil {
		t.Fatal("expected negative error")
	}
}

func TestParsePerformedOn(t *testing.T) {
	got, err := parsePerformedOn("2026-08-24")
	if err != nil || !got.Equal(time.Date(2026, 8, 24, 0, 0, 0, 0, time.UTC)) {
		t.Fatalf("got=%v err=%v", got, err)
	}
	if _, err := parsePerformedOn(""); err == nil {
		t.Fatal("expected missing date error")
	}
	if _, err := parsePerformedOn("24-08-2026"); err == nil {
...
func TestOriginMiddleware(t *testing.T) {
	...
	allowed.Header.Set("Origin", "http://localhost")
	w := httptest.NewRecorder()
	handler.ServeHTTP(w, allowed)
	if w.Code != http.StatusNoContent {
		t.Fatalf("same origin status=%d", w.Code)
	}
